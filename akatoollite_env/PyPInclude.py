...
def include_dir(project, dir = "include", all_ext_allow = False, root = True, error = False, ret = []):
    ...
    if not all_ext_allow:
        for f in ls(target):
            if __is_dir(f):
                if error: include_dir(project, dir = __join_path(dir, f))
                else: include_dir(project, dir = __join_path(dir, f), ret = ret)
            elif __is_link(f):
